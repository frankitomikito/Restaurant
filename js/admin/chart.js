...
async function getReceipts(month) {
    const result = await fetch(`${RequestPath.getPath()}/api/order.php?month=${month}`);
    if (result.ok) {
...
async function getSalesMenu(date) {
    const result =  await fetch(`${RequestPath.getPath()}/api/order.php?date=${date}`);
    if (result.ok) {
        return await result.json();
    } else {
        return [];
...
async function getSalesOrderFromTo(date_from, date_to) {
    const result = 
    await fetch(`${RequestPath.getPath()}/api/receipt.php?date_from=${date_from}&date_to=${date_to}`);
    if (result.ok) {
        return await result.json();
    } else {
        return [];
